...
const Login = (props) => {
  ...
  useEffect(() => { // это хук помогает управлять таймером
    ...
    // clean up function - возвр useEffect
    return () => {
      clearTimeout(timer);
    };
  }, [enteredEmail, enteredPassword]); //return сост дано в качестве зависимости,
     // если изменяется эти зависимости, сработает функция внутри useEffect
